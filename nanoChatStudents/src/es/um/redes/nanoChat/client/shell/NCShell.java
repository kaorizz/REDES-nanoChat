...
public class NCShell {
	...
	private String[] readChatCommandFromStdIn(NCConnector ncclient) {
		...
		while (true) {
			...
			case NCCommands.COM_SEND:
				...
				if (message.toString().length() > 0) vargs.add(message.toString());
				break;
			case NCCommands.COM_PRIVMESSAGE:
				if (st.hasMoreTokens()) vargs.add(st.nextToken(" "));
				if (st.hasMoreTokens()) {
					StringBuffer message2 = new StringBuffer();
					while (st.hasMoreTokens()) message2.append(st.nextToken()+" ");
					if (message2.toString().length() > 0) vargs.add(message2.toString());
				}
			case NCCommands.COM_RENAME:
				StringBuffer message3 = new StringBuffer();
				while (st.hasMoreTokens()) message3.append(st.nextToken()+" ");
				if (message3.toString().length() > 0) vargs.add(message3.toString());
			default:
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NCCommands.COM_SEND:
			...
			break;
		//privmessage requiere los parámetros <user> y <message>
		case NCCommands.COM_PRIVMESSAGE:
			if (args.length <= 1 || args.length > 2) {
				System.out.println("Correct use: rename <user> <message>");
				return false;
			}
		default:
		}
